...
import { Activity, Brain, Dumbbell, Calendar, Zap, Wallet, Compass } from 'lucide-react';
...
export default function StatisticsPage() {
  ...
  const overviewStats = useMemo(() => {
    ...
    const totalCompletedTasks = data.tasks.filter(
      (t) => t.isCompleted && inWindow(t.updatedAt)
    ).length;
  ...
  const resourceScores = useMemo(() => {
    ...
    const social =
      totalPeople > 0
        ? Math.max(10, Math.round(((totalPeople - decayingCount) / totalPeople) * 100))
        : 100;
    ...
    const physical =
      targetExerciseMin > 0
        ? Math.max(
            10,
            Math.min(100, Math.round((overviewStats.totalExerciseMin / targetExerciseMin) * 100))
          )
        : 10;
    ...
    const habitScore =
      targetHabits > 0 ? (overviewStats.completedHabitsCount / targetHabits) * 100 : 100;
  ...
  const recommendations = useMemo(() => {
    ...
    const decayingPeople = data.people
      .filter(isDecaying)
      .slice(0, 2)
      .map((p) => p.name);
  ...
  return (
    <div className="fade-in-entry stats-page">
      <div className="stats-header">
        <div>
          <h2 className="stats-header-title">{t('stats.cross_module_analytics')}</h2>
          <p className="stats-header-subtitle">{t('stats.subtitle')}</p>
        </div>
      ...
      <div className="stats-overview-grid">
        <StatCard
          ...
          subtitle={
            data.fatigue >= 80
              ? t('stats.critical_exhaustion')
              : data.fatigue >= 50
                ? t('stats.elevated_fatigue')
                : t('stats.optimal_state')
          }
          icon={<Brain size={20} />}
      ...
      <div className="glass-panel stats-diagnostic-panel">
        <div>
          ...
          <div className="flex-col-16">
            {[
              {
                label: t('stats.resource.cognitive'),
                val: resourceScores.cognitive,
                color: 'var(--primary)',
                desc: t('stats.cognitive_desc'),
              },
              {
                label: t('stats.resource.social'),
                val: resourceScores.social,
                color: 'var(--blue, #82b1ff)',
                desc: t('stats.social_desc'),
              },
              {
                label: t('stats.resource.finance'),
                val: resourceScores.finance,
                color: 'var(--success, #81c784)',
                desc: t('stats.finance_desc'),
              },
              {
                label: t('stats.resource.physical'),
                val: resourceScores.physical,
                color: 'var(--warning, #fbbf24)',
                desc: t('stats.physical_desc'),
              },
              {
                label: t('stats.resource.mindful'),
                val: resourceScores.mindful,
                color: 'var(--error, #f2b8b5)',
                desc: t('stats.mindful_desc'),
              },
            ].map((item) => (
