        ...
        {data.routes.length > 0 ? (
          <DataTable
            columns={[
              {
                key: 'name',
                label: t('cycling.routes.colName'),
                render: (v) => (
              ...
              {
                key: 'distanceKm',
                label: t('cycling.routes.colDistance'),
                render: (v) => `${(v as number).toFixed(1)} ${t('cycling.common.km')}`,
              },
              {
                key: 'elevationGainM',
                label: t('cycling.dashboard.elevationGain'),
                render: (v) => `${(v as number).toFixed(1)} ${t('cycling.common.m')}`,
              },
              {
                key: 'difficulty',
                label: t('cycling.routes.colDifficulty'),
                render: (v) => (
                  <span
                    className="badge"
                    style={{
              ...
              {
                key: 'waypoints',
                label: t('cycling.routes.colWaypoints'),
                render: (v) => {
              ...
              {
                key: 'isCompleted',
                label: t('cycling.routes.colStatus'),
                render: (v) => (
                  <div
                    style={{
                      display: 'flex',
              ...
              {
                key: 'id',
                label: t('cycling.routes.colActions'),
                render: (_, row) => (
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      className="btn btn--secondary"
                      style={{ padding: '4px 6px' }}
                      onClick={() => handleEdit(row as CycleRoute)}
                    >
                      <Edit2 size={12} />
                    </button>
                    <button
                      className="btn btn--secondary btn-padding-4-6-red"
                      onClick={() => setRouteToDelete((row as CycleRoute).id)}
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
