...
function mapStatus(val: unknown): PersonStatus {
  ...
  if (val === 'На расстоянии' || val === 'distant' || val === 'С ограничениями')
    return PersonStatus.DISTANT;
...
function sanitizePerson(p: unknown): Person {
  ...
  return {
    ...
    reciprocity:
      typeof obj?.reciprocity === 'number' ? Math.max(0, Math.min(100, obj.reciprocity)) : 50,
    volatility:
      typeof obj?.volatility === 'number' ? Math.max(0, Math.min(100, obj.volatility)) : 20,
    lastContactISO:
      typeof obj?.lastContactISO === 'string'
        ? obj.lastContactISO
...
function sanitizeTransaction(tx: unknown): Transaction {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : String(obj?.id || `tx_${Math.random().toString(36).slice(2, 8)}`),
...
function sanitizeReminder(r: unknown): BillReminder {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : String(obj?.id || `rem_${Math.random().toString(36).slice(2, 8)}`),
    title: typeof obj?.title === 'string' && obj.title ? obj.title : 'Напоминание',
    amount: typeof obj?.amount === 'number' ? Math.max(1, obj.amount) : 100,
    dueDateISO:
      typeof obj?.dueDateISO === 'string' ? obj.dueDateISO : new Date().toISOString().slice(0, 10),
    isPaid: typeof obj?.isPaid === 'boolean' ? obj.isPaid : false,
    category: typeof obj?.category === 'string' && obj.category ? obj.category : 'Платежи',
    remindDaysBefore:
      typeof obj?.remindDaysBefore === 'number' ? Math.max(0, obj.remindDaysBefore) : 3,
    createdAt: typeof obj?.createdAt === 'string' ? obj.createdAt : new Date().toISOString(),
    updatedAt:
      typeof obj?.updatedAt === 'string'
        ? obj.updatedAt
...
function sanitizeRide(r: unknown): RideRecord {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `r_${Math.random().toString(36).slice(2, 8)}`,
    dateISO: typeof obj?.dateISO === 'string' ? obj.dateISO : new Date().toISOString().slice(0, 10),
...
function sanitizeRoute(rt: unknown): CycleRoute {
  ...
  const difficultyVal: CycleRoute['difficulty'] = ['easy', 'medium', 'hard', 'extreme'].includes(
    difficultyStr
  )
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `rt_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeMaintenance(m: unknown): MaintenanceRecord {
  ...
  const typeVal: MaintenanceRecord['type'] = [
    'repair',
    'replace',
    'service',
    'upgrade',
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `maint_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeGalleryNote(n: unknown): GalleryNote {
  ...
  const colorVal: GalleryNote['color'] = ['blue', 'green', 'orange', 'pink', 'purple'].includes(
    colorStr
  )
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `gall_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeJournal(j: unknown): JournalEntry {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `journ_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeKnowledge(k: unknown): KnowledgeItem {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `know_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeSchedule(s: unknown): ScheduleBlock {
  ...
  const typeStr = typeof obj?.type === 'string' ? obj.type : '';
  const typeVal: ScheduleBlock['type'] = [
    'work',
    'personal',
    'health',
    'social',
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `sb_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeHabit(h: unknown): Habit {
  ...
  const categoryVal: Habit['category'] = [
    'social',
    'health',
    'mind',
    'productivity',
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `habit_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeWorkout(w: unknown): WorkoutRecord {
  ...
  const typeStr = typeof obj?.type === 'string' ? obj.type : '';
  const typeVal: WorkoutRecord['type'] = [
    'running',
    'swimming',
    'gym',
    'yoga',
  ...
  const intensityVal =
    typeof obj?.intensity === 'number' && [1, 2, 3, 4, 5].includes(obj.intensity)
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `work_${Math.random().toString(36).slice(2, 8)}`,
    dateISO: typeof obj?.dateISO === 'string' ? obj.dateISO : new Date().toISOString().slice(0, 10),
    type: typeVal,
...
function sanitizeThought(t: unknown): Thought {
  ...
  return {
    id:
      typeof obj?.id === 'string' && obj.id
        ? obj.id
        : `thou_${Math.random().toString(36).slice(2, 8)}`,
...
function sanitizeData(data: unknown): AppData {
  ...
  const themeVal = ['mindveyz', 'cyclist', 'reflect', 'slate'].includes(themeStr)
    ? (themeStr as AppData['settings']['theme'])
    : def.settings.theme;

  const accentStr = typeof rawSettings.accentColor === 'string' ? rawSettings.accentColor : '';
  const accentVal = ['purple', 'orange', 'green', 'blue', 'rose'].includes(accentStr)
    ? (accentStr as AppData['settings']['accentColor'])
    : def.settings.accentColor;

  const themeModeStr = typeof rawSettings.themeMode === 'string' ? rawSettings.themeMode : '';
  const themeModeVal = ['manual', 'adaptive', 'system'].includes(themeModeStr)
    ? (themeModeStr as AppData['settings']['themeMode'])
    : def.settings.themeMode;
  ...
  return {
    ...
    settings: {
      ...
      fontSizeScale:
        typeof rawSettings.fontSizeScale === 'number'
          ? Math.max(0.8, Math.min(1.2, rawSettings.fontSizeScale))
          : def.settings.fontSizeScale,
      isAdaptive:
        typeof rawSettings.isAdaptive === 'boolean'
          ? rawSettings.isAdaptive
          : def.settings.isAdaptive,
      graphSensitivity:
        typeof rawSettings.graphSensitivity === 'number'
          ? Math.max(1, Math.min(10, rawSettings.graphSensitivity))
          : def.settings.graphSensitivity,
      graphWeights: {
        energy:
          typeof rawWeights.energy === 'number'
            ? Math.max(0, Math.min(1, rawWeights.energy))
            : def.settings.graphWeights.energy,
        resonance:
          typeof rawWeights.resonance === 'number'
            ? Math.max(0, Math.min(1, rawWeights.resonance))
            : def.settings.graphWeights.resonance,
        reciprocity:
          typeof rawWeights.reciprocity === 'number'
            ? Math.max(0, Math.min(1, rawWeights.reciprocity))
            : def.settings.graphWeights.reciprocity,
        volatility:
          typeof rawWeights.volatility === 'number'
            ? Math.max(0, Math.min(1, rawWeights.volatility))
            : def.settings.graphWeights.volatility,
        recency:
          typeof rawWeights.recency === 'number'
            ? Math.max(0, Math.min(1, rawWeights.recency))
            : def.settings.graphWeights.recency,
      },
      weekStartDay:
    ...
    transactions: Array.isArray(obj.transactions)
      ? obj.transactions.map(sanitizeTransaction)
      : def.transactions,
    ...
    maintenance: Array.isArray(obj.maintenance)
      ? obj.maintenance.map(sanitizeMaintenance)
      : def.maintenance,
    galleryNotes: Array.isArray(obj.galleryNotes)
      ? obj.galleryNotes.map(sanitizeGalleryNote)
      : def.galleryNotes,
    ...
    fatigue:
      typeof obj.fatigue === 'number' ? Math.max(0, Math.min(100, obj.fatigue)) : def.fatigue,
  };
}
    ...
    try {
      ...
      if (parsed && parsed.settings && typeof parsed.settings === 'object') {
        ...
        if (
          ...
          ['purple', 'orange', 'green', 'blue', 'rose'].includes(s.accentColor)
        ) {
      ...
      if (s && typeof s === 'object') {
        if (Array.isArray(s.tasks) && s.tasks.length > 0) {
          data.tasks = s.tasks.map((t: LegacySyntharaTask) => ({
            ...
            emotion:
              typeof t.emotion === 'number'
                ? Math.round(t.emotion * (t.emotion <= 1.0 ? 100 : 1))
                : 50,
            urgency:
              typeof t.urgency === 'number'
                ? Math.round(t.urgency * (t.urgency <= 1.0 ? 100 : 1))
                : 50,
        ...
        if (Array.isArray(s.scheduleBlocks) && s.scheduleBlocks.length > 0) {
          data.schedule = s.scheduleBlocks.map((sb: LegacySyntharaSchedule) => ({
            ...
            type:
              sb.type &&
              ['work', 'personal', 'health', 'social', 'learning', 'rest'].includes(sb.type)
                ? (sb.type as ScheduleBlock['type'])
                : 'personal',
            isCompleted: false,
            tags: [],
            createdAt: new Date().toISOString(),
        ...
        if (Array.isArray(s.habits) && s.habits.length > 0) {
          data.habits = s.habits.map((h: LegacySyntharaHabit) => ({
            ...
            category:
              h.category &&
              ['social', 'health', 'mind', 'productivity', 'other'].includes(h.category)
                ? (h.category as Habit['category'])
                : 'health',
        ...
        if (Array.isArray(s.workoutRecords) && s.workoutRecords.length > 0) {
          data.workouts = s.workoutRecords.map((w: LegacySyntharaWorkout) => ({
            ...
            type:
              w.type && ['running', 'swimming', 'gym', 'yoga', 'walking', 'other'].includes(w.type)
                ? (w.type as WorkoutRecord['type'])
                : 'other',
            durationMin: w.duration || 30,
            intensity:
              w.intensity && [1, 2, 3, 4, 5].includes(w.intensity)
                ? (w.intensity as WorkoutRecord['intensity'])
                : 3,
      ...
      if (c && typeof c === 'object') {
        ...
        if (Array.isArray(c.maintenance) && c.maintenance.length > 0) {
          data.maintenance = c.maintenance.map((m: LegacyCyclistMaint) => ({
            ...
            type:
              m.type &&
              ['inspection', 'cleaning', 'service', 'repair', 'replace', 'upgrade'].includes(m.type)
                ? (m.type as MaintenanceRecord['type'])
                : 'service',
